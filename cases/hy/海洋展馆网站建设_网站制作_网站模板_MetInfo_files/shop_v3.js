...
function stockPrice(){
	...
	$.each(stockjson,function(i,item){
		if(item.valuelist==str){
			...
			// 规格图片切换
			var $img_current=$('#met-imgs-carousel div.slick-active img'),
				img_x=$('#met-imgs-carousel').attr('data-width'),
				img_y=$('#met-imgs-carousel').attr('data-height'),
				para_img=item.para_img?'../include/thumb.php?dir='+item.para_img+'&x='+img_x+'&y='+img_y:'',
				img_current_src=$img_current.attr('src'),
				img_current_datasrc=$img_current.data('src');
				
			if(para_img){
                $img_current.attr({src:$('.met-showproduct.pagetype1').length?para_img:item.para_img});
				$img_current.parent('span').parent('.lg-item-box')
					.attr({'data-src':item.para_img})
					.attr({'data-exthumbimage':item.para_img});
			}else{
				$img_current.attr({src:img_current_datasrc});
				$img_current.parent('span').parent('.lg-item-box')
					.attr({'data-src':img_current_datasrc})
					.attr({'data-exthumbimage':img_current_datasrc});
			}
		}
	});
}
